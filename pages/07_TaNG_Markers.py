...
#    dfTaNGChr = dfTaNG.loc[(dfTaNG['Chr'] == chromosomeClean)]
    ...
    dfTaNGChr2 = dfTaNGChr[['probeset_id', 'Affx Code', 'Call Rate', 'ConversionType', 'MAF', 'Chr', 'Pos', 'Traffic Light', '35K Breeders Array', 'Allele A', 'Allele B']]
...
with col2:
    ...
    st.write('Using this page, one can visual the distribution of all the TaNG v 1.1 Array markers with respect to the Chinese Spring genome')
    st.write('One can enter a single SNP marker code (e.g., AX-108750802) or upload a csv file containing a list on markers from a single chromosome.')
    st.write('The format of the csv file should be:')
    st.write('Marker')
    st.write('AX-643833569')
    st.write('AX-643869398')
    st.write('AX-95250140')
    st.write('Upload the file using the browse button and then press the "submit" button.')
